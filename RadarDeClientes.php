<?php
// ─────────────────────────────────────────────────────────────────────────────
// TENDÊNCIA DE COMPRAS — versão 100% PHP (substitui o app Streamlit)
// Análises: Clientes alvo (queda) · Clientes sem compras · Índice de risco ·
//           Comparativo por Período
// FONTE DE DADOS: arquivo Excel (dados.xlsx) exportado do WinThor — NÃO acessa
// o Oracle diretamente. Gráficos ApexCharts (CDN), Excel via SheetJS (CDN).
// Só precisa do WAMP (extensões zip + simplexml, habilitadas por padrão).
// ─────────────────────────────────────────────────────────────────────────────
ini_set('display_errors', 1);
error_reporting(E_ALL);
ini_set('memory_limit', '1024M');   // dataset de ~3 anos (254k+ linhas) é grande
set_time_limit(300);

// Menu padrão do sistema — incluído no topo (antes de qualquer saída HTML).
require_once __DIR__ . '/menu.php';

// ── Permissão de acesso à página / escopos de carteira ──────────────────────
// 'radarclientes.php'      → carteira ATACADO (supervisor 4)
// 'radarclientes_varejo'   → carteira VAREJO  (supervisores 2 e 8)
// Quem tem as duas chaves (ou é admin) alterna pelo filtro "Carteira".
// Os escopos seguem SOMENTE as permissões marcadas (admin inclusive)
$escoposDisponiveis = [];
...
// ─────────────────────────────────────────────────────────────────────────────
// FONTE DE DADOS — arquivo Excel exportado do WinThor (substitui o Oracle)
// Reexporte o dados.xlsx periodicamente; a app lê SOMENTE este arquivo.
// Forçar releitura (ignora o cache do parse): acessar a página com ?atualizar=1
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Leitor de .xlsx sem dependências externas (usa as extensões zip + simplexml,
 * habilitadas por padrão no WAMP). Retorna array de linhas associativas
 * [CABECALHO => valor]; a 1ª linha da planilha é o cabeçalho.
 */
...
/**
 * Dataset bruto normalizado (mesmo formato que a versão Oracle devolvia),
 * lido de TC_DADOS_FILE. Fica em memória por request e num cache JSON em disco,
 * invalidado automaticamente pela data de modificação do arquivo.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// LOADERS — filtram o dataset do arquivo (antes eram queries no Oracle)
// ─────────────────────────────────────────────────────────────────────────────

/** Dataset principal: 24 meses de histórico + clientes sem pedido, no escopo. */
function tc_load_principal(string $dt_fim_mais1_ddmmyyyy, array $sups): array {
    $sups = array_map('intval', $sups);

    // Janela de 24 meses a partir do 1º dia do mês corrente
    // (equivale ao ADD_MONTHS(TRUNC(SYSDATE,'MM'), -24) da query antiga).
...
/** Dataset do comparativo: 3 anos civis (ano_ref-2 até hoje), no escopo. */
...
// ─────────────────────────────────────────────────────────────────────────────
// PROCESSAMENTO — port fiel do process_periodo_comparativo (app.py)
// ─────────────────────────────────────────────────────────────────────────────
    ...
    if ($pagina === 'alvo') {
        ...
        if ($dtIni >= $dtFim) {
            ...
            if ($erroQ) {
                ...
                // Filtro da página: queda entre -100% e -10%, ou nunca comprou
                ...
                if (!$res) {
                    ...
                    if (!array_filter($yAnterior)) {
                        $avisos[] = ['info', '⚠️ Sem dados de ' . ($anoAtualLbl - 1) . ' no cache — amplie o histórico na query SQL.'];
                    }
...
/* ── Centralização por tabela ──
   Sem compras: Código (1ª) e Dias Inativo (6ª)
   Índice de risco: Freq max Esp, Dias Inativo, Ind Risco e Compra anual (6ª em diante)
   Comparativo: Cód (1ª), valores dos períodos e Var % (6ª em diante) */
...
<?php
// Renderiza o menu padrão do sistema (já incluído no topo do arquivo)
...
<?php
// ─────────────────────────────────────────────────────────────────────────────
// DADOS DO EXPORT EXCEL — mesma estrutura do app.py (uma aba por vendedor,
// título mesclado, subtítulo de período, cabeçalho escuro, zebra, larguras)
// ─────────────────────────────────────────────────────────────────────────────
$tcExport = null;
if ($dadosPagina) {
